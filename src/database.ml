...
let row_to_page row : Page.t option = 
  ...
  let rec aux props_acc p : Page.t = match p with  
    | [] -> { name = page_name; properties = props_acc } 
    | prop::ps when Filename.extension prop = ".md" -> aux ((`Relation (prop, prop)) :: props_acc) ps
    | prop::ps -> aux ((`Text prop) :: props_acc) ps
  in 
    if page_name = "" then None 
    else Some (aux [] row)

let from_csv (abso, _rel) = 
    let extracted_name = Core.(List.drop_last (String.split ~on:' ' Filename.(basename (chop_extension abso)))) in 
    let name = match extracted_name with 
      | Some s -> Core.(String.concat ~sep:" " s)
      | None -> failwith ("Filename generation failed for: " ^ abso) in 
    ...
    let _remove_header = Csv.Rows.next ic in 
      { name; path; pages = Csv.fold_left ~f:(fun acc row -> (row_to_page row)::acc) ~init:[] ic }
  ...
  let rec get_smallest smallest sofar = function 
    ...
    | (abso, rel)::xs -> 
      let len = String.length abso in 
        if len < sofar 
        then get_smallest (abso, rel) len xs 
        else get_smallest smallest sofar xs
  in
    if List.length possible_links = 1 then List.hd possible_links else  
      let first = List.hd possible_links in  
      get_smallest first (String.length (fst first)) possible_links

let gen_relative abs = 
  let lst = String.split_on_char '/' abs in 
  let len = List.length lst in 
    List.iter (fun s -> print_endline ("GEN REL: " ^ s)) lst;
    print_endline abs;
    "./" ^ (List.nth lst (len - 2)) ^ "/" ^ (List.nth lst (len - 1))

let to_html db files = 
  let gen_page (page : Page.t) = 
    let url = "/" ^ (Filename.chop_extension (fst (get_url db page.name files))) ^ ".html" in 
      [%html {|<li><a href="|} url {|">|} [Html.txt page.name] {|</a></li>|}]
  in
  let body = [%html {|
    <div> 
      <h1>|} [Html.txt db.name] {|</h1>
    </div>
    <div>
      <ul>|} (List.map gen_page (List.map Option.get (List.filter Option.is_some db.pages))) {|</ul>
    </div>
  |}] in Html_gen.(emit_page db.path (wrapper db.name body))
